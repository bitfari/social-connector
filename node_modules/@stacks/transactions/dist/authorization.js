...
function serializeSingleSigSpendingCondition(condition) {
    ...
    bufferArray.push(signature_1.serializeMessageSignature(condition.signature));
...
function deserializeSingleSigSpendingCondition(hashMode, bufferReader) {
    ...
    const signature = signature_1.deserializeMessageSignature(bufferReader);
    return {
        hashMode,
        signer,
        nonce,
    ...
    for (const field of fields) {
        switch (field.contents.type) {
            case constants_1.StacksMessageType.PublicKey:
                if (!keys_1.isCompressed(field.contents))
...
function verifySingleSig(condition, initialSigHash, authType) {
    ...
    const addrBytes = types_1.addressFromPublicKeys(0, condition.hashMode, 1, [pubKey]).hash160;
...
function verifyMultiSig(condition, initialSigHash, authType) {
    ...
    if (haveUncompressed && condition.hashMode === constants_1.AddressHashMode.SerializeP2SH)
        throw new errors_1.VerificationError('Uncompressed keys are not allowed in this hash mode');
    const addrBytes = types_1.addressFromPublicKeys(0, condition.hashMode, condition.signaturesRequired, publicKeys).hash160;
    if (addrBytes !== condition.signer)
        throw new errors_1.VerificationError(`Signer hash does not equal hash of public key(s): ${addrBytes} != ${condition.signer}`);
    return curSigHash;
}
...
function setFee(auth, amount) {
    switch (auth.authType) {
        case constants_1.AuthType.Standard:
            const spendingCondition = Object.assign(Object.assign({}, auth.spendingCondition), { fee: common_1.intToBigInt(amount, false) });
            return Object.assign(Object.assign({}, auth), { spendingCondition });
        case constants_1.AuthType.Sponsored:
            const sponsorSpendingCondition = Object.assign(Object.assign({}, auth.sponsorSpendingCondition), { fee: common_1.intToBigInt(amount, false) });
...
function setNonce(auth, nonce) {
    const spendingCondition = Object.assign(Object.assign({}, auth.spendingCondition), { nonce: common_1.intToBigInt(nonce, false) });
    return Object.assign(Object.assign({}, auth), { spendingCondition });
}
exports.setNonce = setNonce;
function setSponsorNonce(auth, nonce) {
    const sponsorSpendingCondition = Object.assign(Object.assign({}, auth.sponsorSpendingCondition), { nonce: common_1.intToBigInt(nonce, false) });
    return Object.assign(Object.assign({}, auth), { sponsorSpendingCondition });
}
exports.setSponsorNonce = setSponsorNonce;
function setSponsor(auth, sponsorSpendingCondition) {
    const sc = Object.assign(Object.assign({}, sponsorSpendingCondition), { nonce: common_1.intToBigInt(sponsorSpendingCondition.nonce, false), fee: common_1.intToBigInt(sponsorSpendingCondition.fee, false) });
