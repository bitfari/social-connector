...
function serializeStacksMessage(message) {
    switch (message.type) {
        ...
        case constants_1.StacksMessageType.TransactionAuthField:
            return signature_1.serializeTransactionAuthField(message);
        case constants_1.StacksMessageType.MessageSignature:
            return signature_1.serializeMessageSignature(message);
    }
}
...
function deserializeStacksMessage(bufferReader, type, listType) {
    switch (type) {
        ...
        case constants_1.StacksMessageType.MessageSignature:
            return signature_1.deserializeMessageSignature(bufferReader);
        default:
...
function addressFromHashMode(hashMode, txVersion, data) {
    const version = common_2.addressHashModeToVersion(hashMode, txVersion);
    return common_2.addressFromVersionHash(version, data);
}
...
function addressFromPublicKeys(version, hashMode, numSigs, publicKeys) {
    ...
    switch (hashMode) {
        case constants_1.AddressHashMode.SerializeP2PKH:
            return common_2.addressFromVersionHash(version, utils_1.hashP2PKH(publicKeys[0].data));
        case constants_1.AddressHashMode.SerializeP2WPKH:
            return common_2.addressFromVersionHash(version, utils_1.hashP2WPKH(publicKeys[0].data));
        case constants_1.AddressHashMode.SerializeP2SH:
            return common_2.addressFromVersionHash(version, utils_1.hashP2SH(numSigs, publicKeys.map(keys_1.serializePublicKey)));
        case constants_1.AddressHashMode.SerializeP2WSH:
            return common_2.addressFromVersionHash(version, utils_1.hashP2WSH(numSigs, publicKeys.map(keys_1.serializePublicKey)));
    }
}
...
function deserializeLPString(bufferReader, prefixBytes, maxLength) {
    ...
    return postcondition_types_1.createLPString(content, prefixBytes, maxLength !== null && maxLength !== void 0 ? maxLength : 128);
}
exports.deserializeLPString = deserializeLPString;
function codeBodyString(content) {
    return postcondition_types_1.createLPString(content, 4, 100000);
}
    ...
    for (let index = 0; index < length; index++) {
        switch (type) {
            ...
            case constants_1.StacksMessageType.PublicKey:
                ...
                break;
            case constants_1.StacksMessageType.TransactionAuthField:
                l.push(signature_1.deserializeTransactionAuthField(bufferReader));
                break;
        }
    }
...
function serializePostCondition(postCondition) {
    ...
    if (postCondition.conditionType === constants_1.PostConditionType.STX ||
        ...
        bufferArray.push(common_1.intToBytes(postCondition.amount, false, 8));
    }
    return bufferArray.concatBuffer();
}
