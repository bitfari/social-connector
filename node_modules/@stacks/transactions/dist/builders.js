...
async function getNonce(address, network) {
    ...
    const url = network
        ? network_1.StacksNetwork.fromNameOrNetwork(network).getAccountApiUrl(address)
...
async function estimateTransfer(transaction, network) {
    ...
    const url = network_1.StacksNetwork.fromNameOrNetwork(derivedNetwork).getTransferFeeEstimateApiUrl();
    const response = await common_1.fetchPrivate(url, fetchOptions);
    if (!response.ok) {
        let msg = '';
        try {
            msg = await response.text();
...
async function estimateTransaction(transactionPayload, estimatedLen, network) {
    const options = {
        ...
        body: JSON.stringify(Object.assign({ transaction_payload: payload_1.serializePayload(transactionPayload).toString('hex') }, (estimatedLen ? { estimated_len: estimatedLen } : {}))),
    };
    const defaultNetwork = new network_1.StacksMainnet();
    const url = network
        ? network_1.StacksNetwork.fromNameOrNetwork(network).getTransactionFeeEstimateApiUrl()
        : defaultNetwork.getTransactionFeeEstimateApiUrl();
    const response = await common_1.fetchPrivate(url, options);
    if (!response.ok) {
        let msg = '';
        try {
            msg = await response.text();
...
async function broadcastTransaction(transaction, network, attachment) {
    ...
    const derivedNetwork = network !== null && network !== void 0 ? network : deriveNetwork(transaction);
    const url = network_1.StacksNetwork.fromNameOrNetwork(derivedNetwork).getBroadcastApiUrl();
...
async function getAbi(address, contractName, network) {
    ...
    const url = network_1.StacksNetwork.fromNameOrNetwork(network).getAbiApiUrl(address, contractName);
    const response = await common_1.fetchPrivate(url, options);
    if (!response.ok) {
...
async function makeUnsignedSTXTokenTransfer(txOptions) {
    ...
    if (options.sponsored) {
        authorization = authorization_1.createSponsoredAuth(spendingCondition);
    }
    else {
        authorization = authorization_1.createStandardAuth(spendingCondition);
    }
...
async function makeContractDeploy(txOptions) {
    const privKey = keys_1.createStacksPrivateKey(txOptions.senderKey);
    const stacksPublicKey = keys_1.getPublicKey(privKey);
    const publicKey = keys_1.publicKeyToString(stacksPublicKey);
...
async function makeUnsignedContractDeploy(txOptions) {
    ...
    const pubKey = keys_1.createStacksPublicKey(options.publicKey);
    let authorization = null;
...
function makeStandardSTXPostCondition(address, conditionCode, amount) {
    return postcondition_1.createSTXPostCondition(postcondition_types_1.createStandardPrincipal(address), conditionCode, amount);
}
...
function makeContractSTXPostCondition(address, contractName, conditionCode, amount) {
    return postcondition_1.createSTXPostCondition(postcondition_types_1.createContractPrincipal(address, contractName), conditionCode, amount);
}
...
function makeStandardFungiblePostCondition(address, conditionCode, amount, assetInfo) {
    return postcondition_1.createFungiblePostCondition(postcondition_types_1.createStandardPrincipal(address), conditionCode, amount, assetInfo);
}
...
function makeContractFungiblePostCondition(address, contractName, conditionCode, amount, assetInfo) {
    return postcondition_1.createFungiblePostCondition(postcondition_types_1.createContractPrincipal(address, contractName), conditionCode, amount, assetInfo);
}
...
function makeStandardNonFungiblePostCondition(address, conditionCode, assetInfo, assetName) {
    return postcondition_1.createNonFungiblePostCondition(postcondition_types_1.createStandardPrincipal(address), conditionCode, assetInfo, assetName);
}
...
function makeContractNonFungiblePostCondition(address, contractName, conditionCode, assetInfo, assetName) {
    return postcondition_1.createNonFungiblePostCondition(postcondition_types_1.createContractPrincipal(address, contractName), conditionCode, assetInfo, assetName);
}
