...
function serializeIntCV(cv) {
    const buffer = new bn_js_1.default(cv.value.toString())
        .toTwos(constants_2.CLARITY_INT_SIZE)
        .toArrayLike(common_1.Buffer, 'be', constants_2.CLARITY_INT_BYTE_SIZE);
    return bufferWithTypeID(cv.type, buffer);
}
function serializeUIntCV(cv) {
    const buffer = new bn_js_1.default(cv.value.toString()).toArrayLike(common_1.Buffer, 'be', constants_2.CLARITY_INT_BYTE_SIZE);
    return bufferWithTypeID(cv.type, buffer);
}
...
function serializeTupleCV(cv) {
    ...
    for (const key of lexicographicOrder) {
        const nameWithLength = postcondition_types_1.createLPString(key);
