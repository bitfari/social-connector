...
class UserSession {
    ...
    makeAuthRequest(transitKey, redirectURI, manifestURI, scopes, appDomain, expiresAt = common_1.nextHour().getTime(), extraParams = {}) {
    ...
    getAuthResponseToken() {
        var _a;
        const search = (_a = common_1.getGlobalObject('location', {
    ...
    async handlePendingSignIn(authResponseToken = this.getAuthResponseToken()) {
        ...
        const nameLookupURL = `${coreNode}${constants_1.NAME_LOOKUP_PATH}`;
        const fallbackLookupURLs = [
            `https://stacks-node-api.stacks.co${constants_1.NAME_LOOKUP_PATH}`,
            `https://registrar.stacks.co${constants_1.NAME_LOOKUP_PATH}`,
        ].filter(url => url !== nameLookupURL);
        const isValid = await verification_1.verifyAuthResponse(authResponseToken, nameLookupURL, fallbackLookupURLs);
        if (!isValid) {
            throw new common_1.LoginFailedError('Invalid authentication response.');
        }
        let appPrivateKey = tokenPayload.private_key;
        let coreSessionToken = tokenPayload.core_token;
        if (common_1.isLaterVersion(tokenPayload.version, '1.1.0')) {
            if (transitKey !== undefined && transitKey != null) {
                if (tokenPayload.private_key !== undefined && tokenPayload.private_key !== null) {
                    try {
                        ...
                        try {
                            encryption_1.hexStringToECPair(tokenPayload.private_key);
                        }
                        catch (ecPairError) {
        ...
        if (common_1.isLaterVersion(tokenPayload.version, '1.2.0') &&
            tokenPayload.hubUrl !== null &&
            tokenPayload.hubUrl !== undefined) {
            hubUrl = tokenPayload.hubUrl;
        }
        if (common_1.isLaterVersion(tokenPayload.version, '1.3.0') &&
        ...
        if (!userData.profile && profileURL) {
            const response = await common_1.fetchPrivate(profileURL);
            if (!response.ok) {
                ...
                userData.profile = profile_1.extractProfile(wrappedProfile[0].token);
            }
        }
        else {
    ...
    encryptContent(content, options) {
        ...
        return encryption_1.encryptContent(content, opts);
    }
    decryptContent(content, options) {
        ...
        return encryption_1.decryptContent(content, opts);
    }
