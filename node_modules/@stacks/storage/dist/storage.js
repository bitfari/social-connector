...
class Storage {
    ...
    async getFile(path, options) {
        const defaults = {
            decrypt: true,
            verify: false,
            app: common_1.getGlobalObject('location', { returnEmptyObject: true }).origin,
        };
    ...
    async getUserAppFileUrl(path, username, appOrigin, zoneFileLookupURL) {
        const profile = await auth_1.lookupProfile({ username, zoneFileLookupURL });
        let bucketUrl;
    ...
    async getFileContents(path, app, username, zoneFileLookupURL, forceText) {
        const opts = { app, username, zoneFileLookupURL };
        const readUrl = await this.getFileUrl(path, opts);
        const response = await common_1.fetchPrivate(readUrl);
        if (!response.ok) {
            throw await hub_1.getBlockstackErrorFromResponse(response, `getFile ${path} failed.`, null);
        }
    ...
    async getFileSignedUnencrypted(path, opt) {
        ...
        try {
            ...
            const signerAddress = encryption_1.publicKeyToAddress(publicKey);
            ...
            else if (!encryption_1.verifyECDSA(fileContents, publicKey, signature)) {
    ...
    async handleSignedEncryptedContents(path, storedContents, app, privateKey, username, zoneFileLookupURL) {
        ...
        const appPublicKey = encryption_1.getPublicKeyFromPrivate(appPrivateKey);
        let address;
        if (username) {
            address = await this.getGaiaAddress(app, username, zoneFileLookupURL);
        }
        else {
            address = encryption_1.publicKeyToAddress(appPublicKey);
        }
        if (!address) {
            throw new common_1.SignatureVerificationError('Failed to get gaia address for verification of: ' + `${path}`);
        }
        ...
        const signerAddress = encryption_1.publicKeyToAddress(signerPublicKey);
        ...
        else if (!encryption_1.verifyECDSA(cipherText, signerPublicKey, signature)) {
    ...
    async putFile(path, content, options) {
        ...
        const opt = Object.assign({}, defaults, options);
        const gaiaHubConfig = await this.getOrSetLocalGaiaHubConnection();
        const maxUploadBytes = common_1.megabytesToBytes(gaiaHubConfig.max_file_upload_size_megabytes);
        ...
        if (opt.encrypt && hasMaxUpload) {
            const encryptedSize = encryption_1.eciesGetJsonStringLength({
        ...
        if (!opt.encrypt && opt.sign) {
            ...
            const signatureObject = encryption_1.signECDSA(privateKey, contentData);
            ...
            if (!opt.encrypt && !opt.sign) {
                ...
                if (typeof opt.encrypt === 'string') {
                    ...
                    publicKey = encryption_1.getPublicKeyFromPrivate(opt.sign);
                }
                else {
                    publicKey = encryption_1.getPublicKeyFromPrivate(this.userSession.loadUserData().appPrivateKey);
                }
                const contentData = await contentLoader.load();
            ...
            uploadFn = async (hubConfig) => {
                const writeResponse = await hub_1.uploadToGaiaHub(path, contentForUpload, hubConfig, contentType, newFile, etag, opt.dangerouslyIgnoreEtag);
    ...
    async listFilesLoop(hubConfig, page, callCount, fileCount, callback) {
        if (callCount > 65536) {
        ...
        try {
            ...
            response = await common_1.fetchPrivate(`${hubConfig.server}/list-files/${hubConfig.address}`, fetchOptions);
            if (!response.ok) {
                throw await hub_1.getBlockstackErrorFromResponse(response, 'ListFiles failed.', hubConfig);
            }
        }
        catch (error) {
            if (callCount === 0) {
    ...
    async setLocalGaiaHubConnection() {
        ...
        const gaiaConfig = await hub_1.connectToGaiaHub(userData.hubUrl, userData.appPrivateKey, userData.gaiaAssociationToken);
