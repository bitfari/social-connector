...
function generateTransitKey() {
    const transitKey = encryption_1.makeECPrivateKey();
    return transitKey;
}
exports.generateTransitKey = generateTransitKey;
function makeAuthRequest(transitPrivateKey, redirectURI, manifestURI, scopes = constants_1.DEFAULT_SCOPE.slice(), appDomain, expiresAt = common_2.nextMonth().getTime(), extraParams = {}) {
    const getWindowOrigin = (paramName) => {
        const location = common_2.getGlobalObject('location', {
    ...
    const address = encryption_1.publicKeyToAddress(publicKey);
...
async function encryptPrivateKey(publicKey, privateKey) {
    const encryptedObj = await encryption_1.encryptECIES(publicKey, common_1.Buffer.from(privateKey), true);
...
async function decryptPrivateKey(privateKey, hexedEncrypted) {
    ...
    const decrypted = await encryption_1.decryptECIES(privateKey, encryptedObj);
...
async function makeAuthResponse(privateKey, profile = {}, username = null, metadata, coreToken = null, appPrivateKey = null, expiresAt = common_2.nextMonth().getTime(), transitPublicKey = null, hubUrl = null, blockstackAPIUrl = null, associationToken = null) {
