...
async function fetchPrivate(input, init) {
    ...
    const fetchResult = await (0, cross_fetch_1.default)(input, fetchOpts);
    return fetchResult;
}
...
function hexToCV(hex) {
    return (0, clarity_2.deserializeCV)(hex);
}
...
const validateTxId = (txid) => {
    if (txid === 'success')
        return true;
    const value = (0, common_2.with0x)(txid).toLowerCase();
    if (value.length !== 66)
        return false;
    return (0, common_2.with0x)(BigInt(value).toString(16).padStart(64, '0')) === value;
};
